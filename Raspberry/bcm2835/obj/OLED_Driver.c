...
#elif USE_SPI_3W
    OLED_CS_0;
    SPI3W_Write_Byte(Reg, SPI_3W_CMD);
    OLED_CS_1;
#elif USE_IIC
...
#elif USE_SPI_3W
//	OLED_CS_0;
    SPI3W_Write_Byte(Data, SPI_3W_RAM);
//	OLED_CS_1;
#elif USE_IIC
...
#if 0
static void OLED_WriteData_16Bit(uint16_t Data)
{
#if USE_SPI_4W
    OLED_DC_1;
    OLED_CS_0;
...
#elif USE_SPI_3W
//	OLED_CS_0;
    SPI3W_Write_Byte(Data >> 8, SPI_3W_RAM);
    SPI3W_Write_Byte(Data, SPI_3W_RAM);
//	OLED_CS_1;
#elif USE_IIC
    I2C_Write_Byte(Data >> 8,IIC_RAM);
    I2C_Write_Byte(Data,IIC_RAM);
#endif
}
#endif
...
void OLED_Display(void)
{
    ...
    OLED_SetWindow(0, 0, sOLED_DIS.OLED_Dis_Column, sOLED_DIS.OLED_Dis_Page);
    //write data
    //for (page = 0; page < sOLED_DIS.OLED_Dis_Page; page = page + 2) {
	for (page = 0; page < sOLED_DIS.OLED_Dis_Page; page++) {
        for(Column = 0; Column < sOLED_DIS.OLED_Dis_Column / 2; Column++ ) {
			//OLED_WriteData((Buffer[Column + page * 64] & 0xf0) | (0x0f & Buffer[Column + (page + 1) * 64]));
			//OLED_WriteData((Buffer[Column + page * 64] & 0x0f) | (0xf0 & Buffer[Column + (page + 1) * 64]));
